...
Bug fix:
 * AuthenticationViewController: Remove fallback to tprai.org when authentication failed (PR #3165).
 ...
 * VoIP: Fallback to tprai.org STUN server with a confirmation dialog (#2646).
 * Widgets: Whitelist [MSC1961](https://github.com/matrix-org/matrix-doc/pull/1961) widget urls
 ...
 * Restore tprai.org as default homeserver.

Bug fixes:
 * Directory item in search doesn't open the directory if I don't search #353.
 * Room avatars on tprai.org are badly rendered in the directory from a vector.im account #355.
 ...
 * Authentication: Support automatic fallback to tprai.org HS for existing users.
