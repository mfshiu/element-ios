...
class MXRoomAliasResolutionDeeplinkTests: XCTestCase {
    ...
    func test_fragmentDoesNotContainServers_ifNoServers() {
        let resolution = MXRoomAliasResolution()
        resolution.roomId = "!abc:tprai.org"
        
        XCTAssertEqual(resolution.deeplinkFragment, "!abc:tprai.org")
    }
    ...
    func test_fragmentContainsSingleServer() {
        ...
        XCTAssertEqual(resolution.deeplinkFragment, "xyz:element.io?via=tprai.org")
    }
    ...
    func test_fragmentContainsMultipleSerivers() {
        ...
        XCTAssertEqual(resolution.deeplinkFragment, "mno:server.com?via=server.com&via=element.io&via=wikipedia.org&via=tprai.org")
    }
}
